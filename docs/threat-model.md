...
| Prompt injection                 | priority-separated prompt layers, deterministic diagnostic scanner, policy gates | heuristic signals have false positives/negatives and are not authorization |
...
`PromptInjectionScanner` adds observability at untrusted prompt boundaries but does not block or rewrite content. `PermissionEngine` and `PolicyEngine` remain authoritative, and the scanner is not a security boundary. See [prompt-injection signals](prompt-injection.md).
